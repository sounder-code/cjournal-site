...
async function main() {
  ...
  while (created.length < count && keywordQueue.length > 0) {
    ...
    for (const candidate of candidates) {
      ...
      if (countSections(candidate.parsed.content) < 4) {
      ...
      const publishNow = nowKSTIso();
      candidate.parsed.data.publishedAt = publishNow;
      candidate.parsed.data.updatedAt = publishNow;
